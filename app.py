...
class QRCodeApp:
    ...
    def create_widgets(self):
        ...
        # QR style section
        style_frame = ttk.LabelFrame(main_frame, text="QR Style & Colors", padding=10)
        ...
        ttk.Label(style_frame, text="QR Color:").grid(row=2, column=0, sticky="w", padx=5, pady=5)
        ...
        self.fg_color_preview.grid(row=2, column=1, sticky="w", padx=5)

        ttk.Button(
            style_frame,
            text="Pick QR Color",
            command=self.pick_foreground_color
        ).grid(row=2, column=1, padx=55, pady=5, sticky="w")

        # Background color picker
        ttk.Label(style_frame, text="Background Color:").grid(row=3, column=0, sticky="w", padx=5, pady=5)
        ...
        self.bg_color_preview.grid(row=3, column=1, sticky="w", padx=5)

        ttk.Button(
            style_frame,
            text="Pick Background Color",
            command=self.pick_background_color
        ).grid(row=3, column=1, padx=55, pady=5, sticky="w")
        ...
        note_label = ttk.Label(
            main_frame,
            text="Note: For vCard QR, mobile will open Add Contact screen. Direct silent auto-save is not allowed by phone security.",
            foreground="gray"
        )
    ...
    def create_labeled_entry(self, parent, label_text, row):
        # Create label and entry field in one row
    ...
    def toggle_form(self):
        # Show selected form and hide the other form
    ...
    def pick_foreground_color(self):
        # Open color picker for QR foreground color
        color = colorchooser.askcolor(
            title="Pick QR Color",
            initialcolor=self.foreground_color
        )

        if color and color[1]:
            self.foreground_color = color[1]
            self.fg_color_preview.config(background=self.foreground_color)

    def pick_background_color(self):
        # Open color picker for QR background color
        color = colorchooser.askcolor(
        ...
        if color and color[1]:
            ...
            self.bg_color_preview.config(background=self.background_color)

    def reset_colors(self):
        # Reset QR colors to default black and white
        self.foreground_color = "#000000"
        self.background_color = "#FFFFFF"
    ...
    def generate_vcard_data(self):
        # Generate mobile-compatible vCard text
        ...
        # Add https:// automatically if website is entered without protocol
        if website and not website.startswith(("http://", "https://")):
            website = "https://" + website

        # vCard 3.0 is widely supported by Android and iPhone
        vcard = (
            "BEGIN:VCARD\n"
            "VERSION:3.0\n"
            f"N:{full_name};;;;\n"
    ...
    def get_qr_data(self):
        # Return QR data according to selected QR type
        if self.qr_type.get() == "Website":
    ...
    def generate_qr_code(self):
        try:
            ...
            # Create styled QR image with selected colors
            img = qr.make_image(
            ...
            # Save vCard file automatically with QR image
            ...
            if self.qr_type.get() == "vCard":
                messagebox.showinfo(
                    "Success",
                    "QR Code and vCard file saved successfully.\n\nMobile will show Add Contact screen after scanning."
                )
            else:
                messagebox.showinfo(
                    "Success",
    ...
    def save_vcard_only(self):
        try:
            ...
            full_name = self.full_name_entry.get().strip().replace(" ", "_") or "Contact"
    ...
    def show_preview(self, img):
        # Resize only preview image, not the actual saved QR image
        preview_img = img.resize((280, 280))
